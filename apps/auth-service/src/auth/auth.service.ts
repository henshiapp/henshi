...
export class AuthService {
    ...
    async signUp(createUserDto: CreateUserDto): Promise<User> {
        const userExists = await this.usersService.findOne({ email: createUserDto.email });
    ...
    async getAccessToken(user: User) {
        return this.jwtService.signAsync(
            ...
            },
            {
                ...
                expiresIn: this.configService.get('jwt.access.expiration'),
            }
        );
    }
    ...
    async getRefreshToken(userId: string) {
        return this.jwtService.signAsync(
            ...
            },
            {
                ...
                expiresIn: this.configService.get('jwt.refresh.expiration'),
            }
        );
    }
    ...
    async sendEmailConfirmation(userId: string, userName: string, userEmail: string) {
        ...
        const url = `${this.configService.get(
            'client.url'
        )}/verificar-email?token=${verificationToken}`;
